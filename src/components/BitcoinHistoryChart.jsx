...
export default function BitcoinHistoryChart({ bitcoinHistory }) {
    ...
    if (!bitcoinHistory || !bitcoinHistory.data || bitcoinHistory.data.length === 0) {
        return null;
    }

    const history = bitcoinHistory.data;

    // Calculate statistics
    const stats = useMemo(() => {
        const prices = history.map(d => d.price_usd);
        const minPrice = Math.min(...prices);
        const maxPrice = Math.max(...prices);
        const minItem = history.find(d => d.price_usd === minPrice);
        const maxItem = history.find(d => d.price_usd === maxPrice);

        const firstPrice = history[0].price_usd;
        ...
        const yesterdayPrice = history.length > 1 ? history[history.length - 2].price_usd : lastPrice;
        ...
        const dataWithMA = history.map((item, index) => {
            ...
            const avg = last7.reduce((sum, d) => sum + d.price_usd, 0) / 7;
            return { ...item, ma7: avg };
        });
    ...
    const CustomTooltip = ({ active, payload }) => {
        ...
        const prevPrice = history[history.indexOf(data) - 1]?.price_usd || data.price_usd;
        const change = data.price_usd - prevPrice;
        const changePercent = (change / prevPrice) * 100;

        return (
            <div className="gold-tooltip">
                <p className="tooltip-date">{data.date}</p>
                <p className="tooltip-price">${data.price_usd.toLocaleString()}</p>
                {history.indexOf(data) > 0 && (
                    <p className={`tooltip-change ${change >= 0 ? 'positive' : 'negative'}`}>
                        {change >= 0 ? '▲' : '▼'} ${Math.abs(change).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ({changePercent.toFixed(2)}%)
                    </p>
                )}
                {data.ma7 && (
                    <p className="tooltip-ma">7-Day MA: ${data.ma7.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                )}
            </div>
        );
    };
        ...
        <div className="brutal-card gold-history-card animate-slide-in" style={{ borderColor: 'var(--bitcoin-accent)', boxShadow: '4px 4px 0 var(--bitcoin-accent)' }}>
            <div className="card-header">
                <div>
                    ...
                    <h3>BITCOIN PRICE HISTORY (30 DAYS)</h3>
                </div>
            </div>
                    ...
                    >
                        <defs>
                            <linearGradient id="bitcoinGradient" x1="0" y1="0" x2="0" y2="1">
                                <stop offset="5%" stopColor="var(--bitcoin-accent)" stopOpacity={0.7} />
                                <stop offset="95%" stopColor="var(--bitcoin-accent)" stopOpacity={0.1} />
                            </linearGradient>
                        </defs>
                    ...
                    <table className="gold-price-table">
                        ...
                        <tbody>
                            {stats.dataWithMA.slice().reverse().map((item, index) => {
                                ...
                                const prevPrice = actualIndex > 0 ? stats.dataWithMA[actualIndex - 1].price_usd : item.price_usd;
                                const change = item.price_usd - prevPrice;
                                const changePercent = (change / prevPrice) * 100;
                                const isMin = item.price_usd === stats.minPrice;
                                const isMax = item.price_usd === stats.maxPrice;

                                return (
                                    <tr key={item.date} className={`${isMin ? 'min-row' : ''} ${isMax ? 'max-row' : ''}`}>
                                        <td>{item.date}</td>
                                        <td className="price-cell">${item.price_usd.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                                        ...
                                        <td>{item.ma7 ? `$${item.ma7.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : '-'}</td>
                                    </tr>
                                );
                            })}
                        </tbody>
