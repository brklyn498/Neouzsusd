...
const NewsFeed = ({ news, darkMode, selectedTag }) => {
    ...
    <div className="news-feed">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', flexWrap: 'wrap', gap: '0.5rem' }}>
        <h2 style={{ margin: 0, fontSize: '2rem', textTransform: 'uppercase' }}>
          FINANCIAL NEWS
          {selectedTag && (
            <span style={{ fontSize: '0.9rem', marginLeft: '0.75rem', padding: '0.25rem 0.5rem', backgroundColor: '#00FFFF', color: '#000', verticalAlign: 'middle' }}>
              #{selectedTag.toUpperCase()}
            </span>
          )}
        </h2>
        <span style={{ fontSize: '0.8rem', opacity: 0.7 }}>
          {selectedTag ? `${filteredNews.length} of ${news.items.length}` : news.items.length} ARTICLES
        </span>
      </div>
      ...
      {filteredNews.length === 0 ? (
        <div className="brutal-card" style={{ padding: '2rem', textAlign: 'center', backgroundColor: 'var(--card-bg)' }}>
          ...
          <p>No articles found matching "{selectedTag}". Try a different tag.</p>
        </div>
      ) : (
